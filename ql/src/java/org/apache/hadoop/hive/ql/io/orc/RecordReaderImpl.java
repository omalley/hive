...
class RecordReaderImpl implements RecordReader {
  ...
  protected RecordReaderImpl(List<StripeInformation> stripes,
    ...
    reader = createTreeReader(0, types, included, skipCorrupt, columnEncryption,
        encryptionKeys);
    indexes = new OrcProto.RowIndex[types.size()];
    bloomFilterIndices = new OrcProto.BloomFilterIndex[types.size()];
    advanceToNextRow(reader, 0L, true);
  ...
  private static TreeReader createTreeReader(int columnId,
    ...
    if (columnEncryptions[columnId] != null && keys[columnId] == null) {
