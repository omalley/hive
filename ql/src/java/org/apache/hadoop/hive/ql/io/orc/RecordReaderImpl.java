...
class RecordReaderImpl implements RecordReader {
  ...
  private final ReaderImpl.EncryptionKey[] keys;
  ...
  protected RecordReaderImpl(List<StripeInformation> stripes,
    ...
    totalRowCount = rows;
    boolean skipCorrupt = HiveConf.getBoolVar(conf,
        HiveConf.ConfVars.HIVE_ORC_SKIP_CORRUPT_DATA);
    this.keys = keys;
   ...
   * @param columnId the columnId for this column
   * @param statsProto the statistics for the column mentioned in the predicate
   * @param predicate the leaf predicate we need to evaluation
   * @param bloomFilter
   * @param keys the encryption keys for all of the columns
   * @return the set of truth values that may be returned for the given
   *   predicate.
   */
  static TruthValue evaluatePredicateProto(int columnId,
    ...
    public boolean[] pickRowGroups(StripeInformation stripe,
      ...
      for (int rowGroup = 0; rowGroup < result.length; ++rowGroup) {
        for (int pred = 0; pred < leafValues.length; ++pred) {
          int colId = filterColumns[pred];
          if (colId != -1) {
            OrcProto.ColumnStatistics stats =
                indexes[colId].getEntry(rowGroup).getStatistics();
            OrcProto.BloomFilter bf = null;
            if (bloomFilterIndices[colId] != null) {
              bf = bloomFilterIndices[colId].getBloomFilter(rowGroup);
            }
            leafValues[pred] = evaluatePredicateProto(colId, stats,
                sargLeaves.get(pred), bf, keys);
            if (LOG.isDebugEnabled()) {
  ...
  protected boolean[] pickRowGroups() throws IOException {
    ...
    return sargApp.pickRowGroups(stripes.get(currentStripe), indexes, keys);
  }
  ...
  void createStreams(List<OrcProto.Stream> streamDescriptions,
    ...
    for (OrcProto.Stream streamDesc: streamDescriptions) {
      ...
      Cipher cipher = null;
      if (streamDesc.hasIv() && keys[column] != null) {
        cipher = Cipher.Factory.get(keys[column].getAlgorithm());
        cipher.initialize(Cipher.Mode.DECRYPT, keys[column].getKey(),
            streamDesc.getIv().asReadOnlyByteBuffer(), 0);
      }
